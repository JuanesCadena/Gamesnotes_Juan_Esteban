    ...
    <div class="form-container" style="margin-top: 5rem">
        ...
        <form method="POST" action="{{ route('admin.video_games.update', ['id' => $videoGame->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @if($videoGame->cover)
                    <p class="text-muted">Portada actual: <a href="{{ asset($videoGame->cover) }}" target="_blank">{{ $videoGame->cover }}</a></p>
                @endif
            </div>
            ...
            <div class="form-group">
                ...
                @if($videoGame->box_art)
                    <p class="text-muted">Carátula actual: <a href="{{ asset($videoGame->box_art) }}" target="_blank">{{ $videoGame->box_art }}</a></p>
                @endif
            </div>
            ...
            <div class="form-group">
                ...
                <div class="checkbox-group">
                    @foreach ($platform as $pla)
                        ...
                        <label for="{{ $pla }}" class="checkbox-label">{{ ucfirst(str_replace('_', ' ', $pla)) }}</label>
                    @endforeach
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                @foreach ($genres as $genre)
                    ...
                    <label for="{{ $genre }}" class="checkbox-label">{{ $genre }}</label>
                @endforeach
            </div>
            @if(session('success'))
